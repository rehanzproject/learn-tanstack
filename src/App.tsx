...
export default function JobFinderApp() {
  ...
  const filteredJobs = useMemo(() => {
    let result = JOBS.filter(job => {
      const namaMatch = !filters.nama || 
        job.title.toLowerCase().includes(filters.nama.toLowerCase()) ||
        job.company.toLowerCase().includes(filters.nama.toLowerCase()) ||
        job.tags.some(tag => tag.toLowerCase().includes(filters.nama.toLowerCase()));
      
      const kotaMatch = filters.kota === 'Semua' || job.city === filters.kota;
      const jenisMatch = filters.jenisPekerjaan === 'Semua' || job.jobType === filters.jenisPekerjaan;
      const tingkatMatch = filters.tingkat === 'Semua' || job.level === filters.tingkat;
      const modeMatch = filters.mode === 'Semua' || job.mode === filters.mode;
      const gajiMatch = !filters.gajiMinimum || job.salaryMax >= parseInt(filters.gajiMinimum);

      return namaMatch && kotaMatch && jenisMatch && tingkatMatch && modeMatch && gajiMatch;
    });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex flex-col">
      <header className="sticky top-0 z-50 bg-white shadow-md">
        <div className="container mx-auto px-4 py-4">
          <h1 className="text-2xl md:text-3xl font-bold text-indigo-600">Job Finder</h1>
          <p className="text-sm text-gray-600 mt-1">Temukan pekerjaan impianmu</p>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Filter Pencarian</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="jenisPekerjaan" className="block text-sm font-medium text-gray-700 mb-1">
                    Jenis Pekerjaan
                  </label>
                  <select
                    id="jenisPekerjaan"
                    value={filters.jenisPekerjaan}
                    onChange={(e) => handleFilterChange('jenisPekerjaan', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  >
                    <option>Semua</option>
                ...
                <div>
                  <label htmlFor="gajiMinimum" className="block text-sm font-medium text-gray-700 mb-1">
                    Gaji Minimum
                  </label>
                  <input
                    id="gajiMinimum"
                    ...
                    onChange={(e) => handleFilterChange('gajiMinimum', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  />
                </div>
          ...
          <section className="lg:col-span-3">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <p className="text-gray-700" role="status" aria-live="polite">
                Menampilkan <span className="font-bold">{filteredJobs.length}</span> lowongan
              </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
                  {paginatedJobs.map(job => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500 mb-4">📍 {job.city}</p>
